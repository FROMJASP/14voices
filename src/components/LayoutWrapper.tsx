...
export function LayoutWrapper({ layout, children }: LayoutWrapperProps) {
  ...
  return (
    <div className={`layout layout--${layoutData.type}`}>
      ...
      <main
        className={`main-content ${paddingClasses[(layoutData.spacing?.contentPadding || 'medium') as keyof typeof paddingClasses]}`}
      >
        <div
          className={`container mx-auto px-4 ${containerClasses[(layoutData.containerWidth || 'standard') as keyof typeof containerClasses]}`}
        >
          {layoutData.sidebar?.enabled ? (
            <div
              className={`flex gap-8 ${layoutData.sidebar.position === 'left' ? 'flex-row-reverse' : ''}`}
            >
      ...
      {layoutData.footer?.style !== 'hidden' && <Footer config={layoutData.footer} />}

      {layoutData.customCSS && <style dangerouslySetInnerHTML={{ __html: layoutData.customCSS }} />}
    </div>
  );
}

...
const WordCountOption = memo(
  ({ item, isSelected, onSelect }: { item: any; isSelected: boolean; onSelect: () => void }) => (
    <label
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        {item.price > 0 && <span className="text-primary font-semibold">+€{item.price}</span>}
      </div>
    </label>
  )
);
  ...
  ({
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        {region.price > 0 && <span className="text-primary font-semibold">+€{region.price}</span>}
      </div>
    </label>
  )
);
...
export const PriceCalculatorForm = memo(function PriceCalculatorForm({
  onProductionChange,
  onReset,
  onAddToCart,
...
PriceCalculatorForm.displayName = 'PriceCalculatorForm';

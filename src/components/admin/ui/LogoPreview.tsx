...
export const LogoPreview: UIFieldClientComponent = (props) => {
  ...
  const logoFont = formData?.branding?.logoFont || 'instrument-serif';
  const logoImage = formData?.branding?.logoImage;
  const logoImageDark = formData?.branding?.logoImageDark;

  // Map font values to actual font families
  const fontMap = {
  ...
  const selectedFont = fontMap[logoFont] || fontMap['instrument-serif'];

  return (
    <div className="field-type">
      <div className="field-label">
        <span style={{ fontSize: '14px', fontWeight: '600', color: '#333' }}>Logo Preview</span>
      </div>
      <div
          ...
          >
            {logoType === 'text' ? (
              <span
                style={{
                  fontSize: '32px',
                  fontWeight: logoFont === 'instrument-serif' ? '400' : '700',
                  color: '#000',
                  letterSpacing: logoFont === 'instrument-serif' ? '-0.02em' : '-0.5px',
              ...
              <span style={{ color: '#999', fontSize: '14px' }}>No logo image selected</span>
            )}
          </div>
        </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          >
            {logoType === 'text' ? (
              ...
              <img
                src={
                  logoImageDark && typeof logoImageDark === 'object' && logoImageDark?.url
                    ? logoImageDark.url
                    : typeof logoImage === 'object' && logoImage?.url
                      ? logoImage.url
                      : ''
                }
                alt="Logo"
              ...
              <span style={{ color: '#666', fontSize: '14px' }}>No logo image selected</span>
            )}
          </div>
        </div>
      </div>

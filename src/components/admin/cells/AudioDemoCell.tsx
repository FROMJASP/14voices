...
export const AudioDemoCell: React.FC<DefaultCellComponentProps> = memo(({ rowData }) => {
  const content = useMemo(() => {
    ...
    if (demos.length === 0) {
      return (
        <span style={{ color: 'var(--theme-text-placeholder)', fontSize: '14px' }}>No demos</span>
      );
    }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {demos.map((demo, index) => (
          <div key={index} style={{ fontSize: '13px' }}>
            <span style={{ fontWeight: '500', color: 'var(--theme-text)' }}>{demo.type}:</span>
            <span style={{ marginLeft: '4px', color: 'var(--theme-success-500)' }}>✓</span>
          </div>
        ))}
      </div>
    );

...
export const CohortCell: React.FC<DefaultCellComponentProps> = ({ cellData }) => {
  if (!cellData) {
    return (
      <span
        style={{
  ...
  const cohortData = typeof cellData === 'object' ? cellData : null;
  ...
  const colorScheme = cohortColors[cohortData.color] || cohortColors.gray;
  ...
  return (
    ...
    >
      {cohortData.name || 'Unnamed Cohort'}
    </span>
  );
};

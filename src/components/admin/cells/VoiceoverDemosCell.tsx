...
export const VoiceoverDemosCell: React.FC<DefaultCellComponentProps> = ({ rowData, cellData }) => {
  ...
  if (totalDemos === 0) {
    return <span style={{ color: '#9ca3af', fontSize: '14px' }}>No demos</span>;
  }

  // Create audio element for primary demo if it exists
  const audioUrl =
    primaryDemo?.url ||
    (typeof primaryDemo === 'object' && primaryDemo?.filename
      ? `/media/${primaryDemo.filename}`
      : null);

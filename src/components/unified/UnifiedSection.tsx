...
import type { ContentSectionData, TwoColumnBlockData } from '@/types/blocks'
...
export function UnifiedSection(props: UnifiedSectionProps) {
  ...
  const backgroundColor = (data as any).backgroundColor || props.backgroundColor || 'white'
  const spacing = props.spacing || 'medium'
  const layout = (data as any).layout || props.layout || 'centered'

  // Two column block with rich text
  if (props.blockType === 'twoColumn' || props.leftColumn || props.rightColumn) {
    const columnRatio = (data as any).columnRatio || props.columnRatio || '50-50'
    
    return (
      <section className={`unified-section ${spacingClasses[spacing]} ${bgClasses[backgroundColor as keyof typeof bgClasses]} ${props.className || ''}`}>
        <div className="container mx-auto px-4">
          <div className={`grid gap-8 md:gap-12 ${ratioClasses[columnRatio as keyof typeof ratioClasses]}`}>
            <div className="prose prose-lg">
              <RichText content={props.leftColumn || (data as any).leftColumn} />
            </div>
            <div className="prose prose-lg">
              <RichText content={props.rightColumn || (data as any).rightColumn} />
            </div>
          </div>
        </div>
      </section>
  ...
  if (layout === 'twoColumn' && ((data as any).media || props.media)) {
    const media = (data as any).media || props.media
    const mediaPosition = (data as any).mediaPosition || props.mediaPosition || 'right'
        ...
        <div className="container mx-auto px-4">
          <div className={`grid md:grid-cols-2 gap-8 md:gap-12 items-center ${
            ...
            <div className="prose prose-lg max-w-none">
              <RichText content={(data as any).content || props.content} />
            </div>
          </div>
        </div>
      </section>
  ...
  return (
    <section className={`unified-section ${spacingClasses[spacing]} ${bgClasses[backgroundColor as keyof typeof bgClasses]} ${props.className || ''}`}>
      <div className="container mx-auto px-4">
        <div className={`prose prose-lg max-w-4xl ${alignmentClasses[layout as keyof typeof alignmentClasses]}`}>
          <RichText content={(data as any).content || props.content} />
        </div>
      </div>
    </section>

...
export function RichText({ content, className }: RichTextProps) {
  ...
  const renderNode = (node: RichTextNode): React.ReactNode => {
    ...
    switch (node.type) {
      ...
      case 'link':
        return (
          <Link href={(node.fields?.url as string) || '#'} target={node.fields?.newTab ? '_blank' : undefined}>
            {children}
          </Link>
        )
  ...
  const renderBlock = (fields: Record<string, unknown>) => {
    ...
    switch (blockType) {
      case 'mediaBlock':
        const media = fields.media as { url: string; alt?: string; width?: number; height?: number } | undefined
        ...
        return (
          <figure className={`my-8 ${alignment === 'full' ? 'mx-0' : alignment === 'center' ? 'mx-auto' : ''}`}>
            <Image
              src={media.url}
              alt={media.alt || ''}
      ...
      case 'buttonBlock': {
        ...
        const alignmentClass = buttonAlignment === 'center' ? 'text-center' : buttonAlignment === 'right' ? 'text-right' : 'text-left'

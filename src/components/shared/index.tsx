...
export function Heading({ as: Component = 'h2', className = '', children }: HeadingProps) {
  ...
  return <Component className={`${sizeClasses[Component]} ${className}`}>{children}</Component>;
}
...
export function Text({ className = '', children }: TextProps) {
  return <p className={`text-base md:text-lg text-gray-600 ${className}`}>{children}</p>;
}

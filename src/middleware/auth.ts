...
export async function withAuth(
  ...
  return async (req: NextRequest, context?: AuthContext) => {
    ...
    try {
      ...
      const enhancedContext: AuthContext = {
        ...context,
        user,
      };
...
export function withRateLimit(
  ...
  return async (req: NextRequest, context?: unknown) => {
    ...
    response.headers.set(
      'X-RateLimit-Reset',
      String(new Date(clientData.timestamp + windowMs).toISOString())
    );
 ...
 */
export function composeMiddleware(...middlewares: Array<(handler: unknown) => unknown>) {

...
export async function POST(req: NextRequest) {
  try {
    ...
    if (header) {
      ...
      if (headerComponent) {
        const headerContent =
          headerComponent.content?.root?.children?.reduce((acc: string, node: ContentNode) => {
    ...
    // Add main content
    const mainContent =
      content?.root?.children?.reduce((acc: string, node: ContentNode) => {
        if (node.type === 'paragraph') {
          const text = node.children?.map((child) => child.text || '').join('');
          return acc + `<p>${text}</p>`;
        }
    ...
    if (footer) {
      ...
      if (footerComponent) {
        ...
        const footerContent =
          footerComponent.content?.root?.children?.reduce((acc: string, node: ContentNode) => {
            if (node.type === 'paragraph') {
              const text = node.children?.map((child) => child.text || '').join('');

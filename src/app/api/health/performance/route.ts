...
export const GET = createApiHandler(
  async () => {
    ...
    const recentApiCalls =
      process.env.NODE_ENV === 'production' ? await getRecentApiMetrics() : mockApiMetrics();
    ...
    return {
      ...
      recommendations: generateRecommendations(report, { ...recentApiCalls, errorRate: 0 }),
...
function determineHealthStatus(
  report: { slowQueries: unknown[] },
  apiMetrics: { avgResponseTime: number }
): string {
...
function calculateAvgQueryTime(slowQueries: Array<{ meanTime: number }>): number {
...
function generateRecommendations(
  report: {
    ...
    connectionStats?: { waiting: number };
  },
  apiMetrics: { avgResponseTime: number; errorRate: number }
): string[] {
  ...
  if (report.unusedIndexes && report.unusedIndexes.length > 10) {
  ...
  if (report.connectionStats && report.connectionStats.waiting > 5) {

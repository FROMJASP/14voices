...
export async function generateMetadata(): Promise<Metadata> {
  ...
  try {
    ...
    if (siteSettings?.favicon && typeof siteSettings.favicon === 'object' && 'url' in siteSettings.favicon) {
      ...
      const iconUrl = faviconUrl.startsWith('blob:') || faviconUrl.includes('.vercel-storage.com') 
        ? faviconUrl 
    ...
    return {
      ...
      description: siteSettings?.defaultSeo?.description || 'Professionele voice-overs voor elk project. Van commercials tot bedrijfsfilms.',
      icons,
      ...
      twitter: {
        card: siteSettings?.twitterCard?.cardType as 'summary' | 'summary_large_image' || 'summary_large_image',

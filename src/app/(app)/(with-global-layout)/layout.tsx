    ...
    async () => {
      const defaultInfoNavbarData: InfoNavbarData = {
        ...
        whatsappTooltip: {
          ...
          message: 'We zijn vaak in de studio aan het werk. Stuur ons eerst een WhatsApp-bericht, dan kunnen we je zo snel mogelijk terugbellen.',
        },
      };
        ...
        if (siteSettings?.topBar) {
          ...
          if (topBar.whatsappTooltip) {
            whatsappTooltip = {
              ...
              message: topBar.whatsappTooltip.message || defaultInfoNavbarData.whatsappTooltip?.message,
            };
            
            // Handle image - can be either a Media object with ID or an object with url
            if (topBar.whatsappTooltip.image) {
              if (typeof topBar.whatsappTooltip.image === 'number' || topBar.whatsappTooltip.image.id) {
          ...
          const result = {
            ...
            quickLinks: (topBar.quickLinks && topBar.quickLinks.length > 0) ? topBar.quickLinks : defaultInfoNavbarData.quickLinks,

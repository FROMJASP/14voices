...
export class MultiTierCache implements CacheStrategy {
  ...
  async set(key: string, value: unknown, ttl?: number): Promise<void> {
    await Promise.all(this.tiers.map((tier) => tier.cache.set(key, value, ttl || tier.ttl)));
  }

  async invalidate(patterns: string[]): Promise<void> {
    await Promise.all(this.tiers.map((tier) => tier.cache.invalidate(patterns)));
  }
}
...
export function createCacheStrategy(
  ...
  options: {
    ...
    dataWriter?:
      ...
      | ((batch: Array<{ key: string; value: unknown }>) => Promise<void>);
  ...
  switch (type) {
    ...
    case 'write-around':
      ...
      if (options.dataWriter.length === 2) {
        return new WriteAround(
          options.cache,
          options.dataWriter as (key: string, value: unknown) => Promise<void>
        );
      }
      throw new Error(
        'write-around cache strategy requires a dataWriter with (key, value) signature'
      );
    ...
    case 'write-behind':
      ...
      if (options.dataWriter.length === 1) {
        return new WriteBehind(
          options.cache,
          options.dataWriter as (batch: Array<{ key: string; value: unknown }>) => Promise<void>,
          options
        );
      }
